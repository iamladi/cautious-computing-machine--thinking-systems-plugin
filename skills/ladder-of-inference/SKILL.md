...
Skip when the user has verifiable data already and just needs to pick among options (use decision-matrix), or when the belief-under-scrutiny is a one-off observation with no durable reasoning chain behind it (no ladder to descend). For structural cause analysis of a system-level pattern rather than an individual belief, route to iceberg-model. For an incoming loaded message or question that may be installing a frame on the user, route to `presupposition-detector` before descending the ladder — descending from a frame the user accepted unconsciously starts the ladder one rung too high.
